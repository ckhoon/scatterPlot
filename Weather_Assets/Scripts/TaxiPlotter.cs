...
public class TaxiPlotter : MonoBehaviour, PlotterInterface
{
	...
	private static int FILTER_PERCENT = 20;
	private static int FILTER_RATE = 100 / FILTER_PERCENT;
	private static int MAX_LAYER = 10;
	private static float TAXI_H = 0.05f;
	private static float TAXI_S = 0.9f;
	private static float TAXI_L = 0.5f;
	private static float TAXI_A = 0.8f;
	private static float OPAQUE = TAXI_A;
	private static float FADE = 0.001f;
	...
	private static string URL = "https://api.data.gov.sg/v1/transport/taxi-availability?date_time=";
	...
	private TaxiPlot plot = new TaxiPlot();
	...
	void Start()
	{
		m_cmdLoadFirstData = new vrCommand("TaxiPlotCmdLoadFirstData", cmdLoadFirstData);
		m_cmdAutoPopulate = new vrCommand("TaxiPlotCmdAutoPopulate", cmdAutoPopulate);
		m_cmdUpdatePlot = new vrCommand("TaxiPlotCmdUpdate", cmdUpdatePlot);
	...
	public bool TurnAlpha(float y)
	{
		if ( RealTimeHolder.activeSelf )
			for ( int i = 0; i < RealTimeHolder.transform.childCount; i++ )
			{
		...
		if ( PointHolder.activeSelf )
			for ( int i = 0; i < PointHolder.transform.childCount; i++ )
			{
				Transform child = PointHolder.transform.GetChild(i);
				Color currentColor = child.GetComponent<Renderer>().material.color;
				if ( y != -1 )
				{
					if ( child.position.y == y )
	...
	private vrValue cmdUpdatePlot(vrValue iVal)
	{
		...
		Debug.Log("Taxi recevied " + listLastData.Count + " @ " + DateTime.Now);

		if ( listLastData.Count != 0 )
		{
			strLastUpdateDt = (string)listLastData.Last()[TaxiPlot.Headers.Timestamp];
			...
			// no averaging for taxi data
			listRealTime.Clear();
			listRealTime = listLastData;

			clearRealTime();
			for ( int i = 0; i < listRealTime.Count; i += FILTER_RATE )
			{
	...
	{
		float z =
		( (float)dataPt[TaxiPlot.Headers.Lat] - minLat )
		/ ( maxLat - minLat );

		float x =
			( (float)dataPt[TaxiPlot.Headers.Lng] - minLng )
			/ ( maxLng - minLng );
		...
		float size = cubeSize * cubeSizeMultipler;
		dataPoint.transform.localScale = new Vector3(size, cubeSize / 4.0f, size);
		string strName = dataPt[TaxiPlot.Headers.Lng].ToString() + "_" + dataPt[TaxiPlot.Headers.Lng].ToString();
		dataPoint.transform.name = strName;

		Color pointColor = Color.HSVToRGB(TAXI_H, TAXI_S, TAXI_L);
		pointColor.a = TAXI_A;
		dataPoint.GetComponent<Renderer>().material.color = pointColor;

		string strDetails = "Lat: " + dataPt[TaxiPlot.Headers.Lat].ToString()
							+ "\nLng: " + dataPt[TaxiPlot.Headers.Lng].ToString()
							+ "\nTime: " + (string)dataPt[TaxiPlot.Headers.Timestamp];
		dataPoint.GetComponent<DataDetails>().fullDetails = strDetails;
		dataPoint.GetComponent<DataDetails>().dimension = "Taxi";
	}
}
