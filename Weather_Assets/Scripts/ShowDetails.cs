...
{
	public GameObject plotterHolder;
		...
		if ( button_select.IsToggled() )
		{
			...
			if ( Physics.Raycast(Avpl.AvplStatic.GetRay(), out hit) )
			{
					...
					&& hitObject.GetComponent<Renderer>().enabled )
				{
					...
					PlotterInterface[] plotters = plotterHolder.GetComponentsInChildren<PlotterInterface>();
					foreach ( PlotterInterface plotter in plotters )
					{
						plotter.TurnAlpha(hitObject.transform.position.y);
					}
				}
				else
				{
